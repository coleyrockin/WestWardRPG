...
> **Status: Active development — 3D engine phase.**
> The renderer, first-road loop, and NPR pipeline are live. The 3D build is not yet the
> default deploy — the Canvas raycaster still ships while the 3D game reaches feature
> parity. The project is early in that parity work; this document is honest about that.
>
...
- **Audio layer** (`src/render3d/audioView.js`) — wind bed, boot footsteps, harmonica sting on spawn, lantern hum, slime hit SFX, map-scrap chime. Web Audio API only, no assets required. Currently at zero; this is the single highest-leverage missing piece.
- **First 10 minutes polish** — per-beat scene quality review (spawn → board → sign → cache → slime → wagon) in a real browser. Slime breathing animation (`stepWorld` sin-pulse), smoke cache plume visibility (`dustSmokePlume.glb`), road sign text, scatter density 620→850.
...
| New features in `src/main.js` (the Canvas raycaster) | It's the behavioral oracle — frozen. Bug fixes only; new gameplay goes into the 3D build. |
