...
const FOOTPRINTS = {
  ...
  heroTownSaloon:  (p) => box(p, 2.15 * p.size, 1.0 * p.size),
  heroTownStore:   (p) => box(p, 1.9 * p.size, 0.95 * p.size),
  heroTownAssay:   (p) => box(p, 1.7 * p.size, 0.9 * p.size),
  productionSaloon: (p) => box(p, 2.75 * p.size, 1.05 * p.size),
  productionStore:  (p) => box(p, 2.35 * p.size, 1.0 * p.size),
  productionAssay:  (p) => box(p, 2.1 * p.size, 0.95 * p.size),
  productionBoardwalk: null,
  windowGlowPanel: null,
  hangingSign: null,
  hitchingRail: (p) => box(p, 2.2 * p.size, 0.28 * p.size),
  barrelCrateCluster: (p) => box(p, 0.9 * p.size, 0.8 * p.size),
  npcSilhouette: null,
  lanternString: null,
  mudRutDecal: null,
  dustSmokePlume: null,
