...
- ~~`update()` runs full physics / AI / weather while modals are open.~~ **Shipped 2026-05-07:** the existing `state.mode !== "playing"` early-return at the top of `update()` now also checks `anyModalOpen()`. Time-of-day, region events, and dynamic region progression were moved after the gate so opening dialogue/codex/quest-outcome/settings/character-sheet/skill-tree/job-board/shop now truly pauses the world. Visual feedback (HUD notice TTL, msg fade, weather drift, player attack-cooldown ticks) still runs above the gate.
- No gamepad support. ~~No update-while-paused gating.~~ ~~No music tracks loaded (the `music` audio bus exists at `audio.js:21` but is never populated).~~ Both shipped (modal pause 2026-05-07; procedural music 2026-05-05; outcome-modulated music 2026-05-08).
...
- Shipped: hand-rolled WebAudio scheduler, **zero audio assets** (stays offline-first):
  ...
  - **Outcome-reactive modulation (2026-05-08).** New `src/musicReactivity.js` exposes `applyOutcomeModulation(profile, narrative)` — pure helper that applies small additive deltas (±2-6 BPM, ±0.005-0.02 gain) per (quest × outcome) on top of a base region profile. Late-chain outcomes apply last so the score drifts toward the most recent decision. Wiring: `audio.js` `startMusicForRegion`/`setMusicRegion` accept an optional `{profile}` override; `main.js` `syncAmbientForRegion` resolves the modulated profile every region crossfade; `confirmQuestOutcomeChoice` fires `refreshMusicForCurrentNarrative()` so a decision lands audibly without waiting for region change. This is the sixth player-visible reactivity surface (HUD echo / vendor / NPC / job board / companion / music) over the same `narrative.questOutcomes` map.
