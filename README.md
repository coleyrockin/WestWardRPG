...
Latest local fast gate: `npm test` reports **947 passing tests across 84 files**. The core systems are present; the current roadmap is focused on turning them into a finished-product vertical slice. Run the verification commands below before committing gameplay changes.
