...
- **Jobs/economy foundation**: Marshal Boone offers deterministic bounty, salvage, courier, patrol, supply-run, rescue, and escort work through an in-world job board with timed bonuses, failure/report-back states, regional job depth, route markers, minimap/HUD/debug visibility, progress tracking, and reward payout.
...
Latest local fast gate: `npm test` reports **335 passing tests across 35 files**. Run the verification commands below before committing gameplay changes.
